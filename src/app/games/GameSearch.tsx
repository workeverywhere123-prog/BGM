    ...
    <div style={{ maxWidth: 560, margin: '0 auto 2rem' }}>
      ...
      {results.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1, textAlign: 'left', marginBottom: '0.5rem' }}>
          {results.map(r => (
            <a key={r.boardlife_id} href={r.boardlife_url} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', padding: '0.7rem 1rem', textDecoration: 'none', background: 'rgba(30,74,52,0.2)', borderLeft: '2px solid var(--gold-dim)' }}>
              {r.thumbnail_url ? (
              ...
              <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.55rem', letterSpacing: '0.1em', color: 'var(--gold-dim)', flexShrink: 0 }}>보드라이브 →</span>
            </a>
          ))}
        </div>
      )}
